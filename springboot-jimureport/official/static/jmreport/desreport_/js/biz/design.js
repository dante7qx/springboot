            ...
            return {
              ...
              /*  moduleTitle: "",*/
               ...
               /* sqlFormValidate:{
                ...
                valueCoordinate:"", //值坐标
                dataLabel:(h) => {
                  return this.reanderTooltip(h,'数据','https://help.jeecg.com/jimureport/chart.html');
                },//数据选项卡名称自定义
                codeLabel:(h) => {
                  return this.reanderTooltip(h,'二维码设置','https://help.jeecg.com/jimureport/code/qrCode.html');
                },//二维码选项卡定义    
                qrCodeLabel:(h) => {
                  return this.reanderTooltip(h,'条形码设置','https://help.jeecg.com/jimureport/code/barCode.html');
                },//条形码选项卡定义
...
function handleSingleSeries(object) {
    ...
    if(object.label){
        ...
        //update-begin---author:wangshuai ---date:20220626  for：[issues/I5CO1P、issues/1100 、issues/1086]图表的数值显示，会连轴名称一起显示------------   
        this.seriesLabelSettings.type = type
        //update-end---author:wangshuai ---date:20220626  for：[issues/I5CO1P、issues/1100 、issues/1086]图表的数值显示，会连轴名称一起显示------------   
    }
}
...
function refreshChart(id, chartOptions, dataSettings, dataList){
    ...
    if( chartType === 'bar.simple' || (chartType.indexOf('line.') !== -1 && chartType!=='line.multi')){
        ...
        for(let i=0;i<legendData.length;i++){
            ...
            let itemstyle = getSeriesItemStyle(seriesConfig, i, name)
            //update-begin---author:wangshuai ---date:20220701  for：多对比柱形图支持line和bar切换，如果是多对比柱形图，第一项是line的话，其他选项都会变成line------------
            //先注释掉，以后有用可以放开
            //singleSeries.type = seriesConfig[i].type
            //update-end---author:wangshuai ---date:20220701  for：多对比柱形图支持line和bar切换，如果是多对比柱形图，第一项是line的话，其他选项都会变成line------------
            let obj = Object.assign({},
                singleSeries,
                itemstyle,{
                    name: name,
