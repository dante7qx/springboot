...
// 预览页面 vue实例
 let rpViewInst = new Vue({
     ...
     data: function () {
      ...
      //update-begin-author:taoyan date:20220302 for: 防止xss js攻击
     watch:{
         queryInfo:{
             deep: true,
             handler(){
                 Object.keys(this.queryInfo).map(key=>{
                     ...
                     // 判断类型 字符串类型才走此逻辑 对于日期类型，数字类型不处理
     ...
     //update-end-author:taoyan date:20220302 for: 防止xss js攻击
     created(){
         this.resetQueryInfo()
         //获取url路径上的是否显示上一页
     ...
     methods:{
         ...
         doReportQuery(){
             ...
             $jm.excelView(view.excelConfigId, requestParam, (result)=> {
                 ...
                 //update-begin----Author:wangshuai----Date:20210918----for:查询之后页数变为第一条
                  this.initPageNo();
                 //update-end----Author:wangshuai----Date:20210918----for:查询之后页数变为第一条
             },(res)=>{
                     ...
                     if(paramValue){
                         if(paramValue.startsWith('=')){
                             ...
                             if(matchArray && matchArray.length==3){
                                 ...
                                 let rowNum = param.ri;
                                 if(matchArray[2]){
                                     rowNum = parseInt(matchArray[2]) - 1;
                                 }
                                 let obj = {}
         ...
         handleQueryChange(query,item){
           if(!query){
             //update-begin---Author:wangshuai---Date:20210909---for:支持参数查询
             item.select = false
             item.queryType = 2
             this.dictCodeSearch("",item)
             //update-end---Author:wangshuai---Date:20210909---for:支持参数查询

             //update-begin---Author:taoyan---Date:20211021--for: 查询下拉框 手动删除选项 查询无效 611108139511156736
             this.queryInfo[item.key] = ''
             //update-end---Author:taoyan---Date:20211021--for: 查询下拉框 手动删除选项 查询无效 611108139511156736
           }else{
             //update-begin---author:wangshuai ---date:20220217  for：[issues/766]下拉框模糊查询显示问题,连续2次搜同一个汉字时 第二次之后就不显示了
             item.select = false
             item.queryType = 2
             this.dictCodeSearch(query,item)
             //update-end---author:wangshuai ---date:20220217  for：[issues/766]下拉框模糊查询显示问题,连续2次搜同一个汉字时 第二次之后就不显示了
           }
         },
         ...
         dictCodeSearch(query,item){
           ...
           if(!select || select==false){
             //update--begin--Author:wangshuai--Date:20211207--for:将浏览器的参数也传递过去，后台查询的时候
             let params = getRequestUrlParam();
             let dataStr={"reportId":configId,"text":query,"key":dbKey,"paramSearch":paramSearch,params:params}
             //update--end--Author:wangshuai--Date:20211207--for:将浏览器的参数也传递过去，后台查询的时候
             $http.get({
       ...
       /**
        * 下拉框单选回调事件
        * @param val 回调的值包含查询的key和查询的value
        */
        handleDictOk(val){
        ...
        xs.loadData(excelData);
         ...
         handleReportQueryInfo(token).then(()=>{
             ...
             var loadExcelView = function(inputparam){
                 ...
                 //update-begin---author:wangshuai ---date:20220412  for：[issues/881]报表下钻时返回上一页查询参数会丢失------------
                 let previousPage = window.localStorage.getItem("previousPage");
                 if(previousPage){
                     let queryInfo = window.localStorage.getItem("params_"+ excelConfigId);
                     if(queryInfo){
                 ...
                 //update-end---author:wangshuai ---date:20220412  for：[issues/881]报表下钻时返回上一页查询参数会丢失------------
                 ...
                 getShareViewUrl(excelConfigId, (result)=> {
                     if(result=="0"){
                         $jm.excelView(excelConfigId,requestParam,(result)=> {
                             ...
                             //设置网页标题
                             document.title = result.name+' - 积木报表, 免费的可视化报表！';
                             var str = result.jsonStr;
                             if(!str){
                                 xs.loadData({});
                                 return;
                             ...
                             // 记录报表名称 导出用
                             rpViewInst.jimuReportName = result.name
                             // 设置样式
                             ...
                             // 设置查询条件默认值后首次进入预览页面查询无数据 再次切换查询条件 分页失效 /issues/610
                                 ...
                                 .onSettingEvent(function (e,param) {
                                     if(e=='pdf'){
                                         ...
                                         xs.getLayerBase64().then(values=>{
                                             ...
                                             //update-begin-author:taoyan date:20211214 for: /issues/I4L9FY 导出pdf,打印多传两个参数
                                             ...
                                             //update-end-author:taoyan date:20211214 for: /issues/I4L9FY 导出pdf,打印多传两个参数

                                             //update-begin-author:wangshuai date:20211214 for: [issues/1208、issues/1286]导出数据查询条件取值错误
                                             // Object.keys(exportDefaultValue).map(key=>{
                                             //   if(!queryParam[key]){
                                             //     queryParam[key] = exportDefaultValue[key]
                                             //   }
                                             // })
                                             //update-end-author:wangshuai date:20211214 for: [issues/1208、issues/1286]导出数据查询条件取值错误
                                             ...
                                             $http.post({
                                                 ...
                                                 success:(result)=>{
                                                     Vue.prototype.$Spin.hide();
                                                     ajaxFileDownload2 (result.data, rpViewInst.jimuReportName+".pdf");
                                                 },
                                                 error:(e)=>{
                                                     Vue.prototype.$Spin.hide();
                                                     xs.tip('导出失败');
                                         ...
                                         if(param.linkIds){
                                             ...
                                             let text = param.text;
                                             //update-beign-author:taoyan date:20211213 for:/issues/I4LLPY 单元格没有数据，为空时，点击单元格不跳转
                                             ...
                                             //update-end-author:taoyan date:20211213 for:/issues/I4LLPY 单元格没有数据，为空时，点击单元格不跳转
                                         }
                                 ...
                                 .onExportExcelAll(function(){
                                     ...
                                     xs.getLayerBase64().then(values=>{
                                         ...
                                         $http.post({
                                             ...
                                             success:(result)=>{
                                                 Vue.prototype.$Spin.hide();
                                                 ajaxFileDownload2 (result.data, rpViewInst.jimuReportName+".xlsx");
                                             },
                                             error:(e)=>{
                                                 Vue.prototype.$Spin.hide();
                                                 xs.tip('导出失败！');
                                             ...
                                             success:(result)=>{
                                                 ...
                                                 if(item.extData && item.extData.dbCode){
                                                     ...
                                                     if(isTiming){
                                                        ...
                                                        //update-begin--Author:wangshuai--Date:20211203--for:地图不定时刷新初始化预览页面的时候也需要刷新一下图表
                                                        refreshData(item,dataMap)
                                                        //update-begin--Author:wangshuai--Date:20211203--for:地图不定时刷新初始化预览页面的时候也需要刷新一下图表
                                                     }
                                                 }
 ...
 * @param filename
 */
function ajaxFileDownload2(data, filename) {
 ...
 */
 function getPageResult(pageNo,pageSize=10,type){
    ...
    // requestParam['onlyPageData'] = '1'
     ...
     $jm.excelView(view.excelConfigId,requestParam,(result)=> {
         var str = result.jsonStr;
         // 下钻功能问题 分页后无效 https://gitee.com/jeecg/JimuReport/issues/I4IZ64
         rpViewInst.saveQueryResultData(result.dataList)
         if(!str){
            Vue.prototype.$Spin.hide();
            return;
...
function preRefreshChart(item,chartOption,dataList,dataMap){
    if(item.extData['chartType']==='graph.simple'){
      ...
      //update-begin--Author:wangshuai--Date:20211025--for:地图控件采用SQL数据源无法定时更新 gitee #I4ELR7
      if(item.extData.dataType!="api"){
        refreshChart(rpViewInst, chartOption, item.extData, dataList)
      }
      //update-end--Author:wangshuai--Date:20211025--for:地图控件采用SQL数据源无法定时更新gitee #I4ELR7
      //TODO api接口请求是否动态刷新
...
function refreshChart(vm, chartOptions, dataSettings, dataList){
    ...
    //update-begin---author:wangshuai ---date:20220412  for：[issues/897]180度仪表盘数据自动刷新无效------------
    if(chartType.indexOf("gauge.simple") != -1){
    //update-end---author:wangshuai ---date:20220412  for：[issues/897]180度仪表盘数据自动刷新无效--------------
        let { axisX, axisY } = dataSettings
        let array = []
        for(let item of dataList){
            array.push({
...
function handleLinkageChartParam(linkData, param, type) {
    if(linkData.parameter){
        ...
        if(arr.length>0){
            ...
            //update-begin---author:wangshuai ---date:20220222  for：[issues/I4R92U、issues/I4OAUS]图标联动失败,tomcat高版本不支持特殊格式，可以使用post请求
            $http.post({
                url: api.getCharData,
                contentType:'json',
            //update-end---author:wangshuai ---date:20220222  for：[issues/I4R92U、issues/I4OAUS]图标联动失败,tomcat高版本不支持特殊格式，可以使用post请求
                ...
                success: (result) => {
                    ...
                    if(dataList){
                        ...
                        //图表刷新
                        //update-begin---author:wangshuai ---date:20220426  for：[issues/I531S7]图表联动，当子表没有数据时，会弹窗数据为空，且子表的数据也会恢复设计时候的默认数据------------
                        refreshChart(rpViewInst, chartOptions, result.extData,[])
                        //update-end---author:wangshuai ---date:20220426  for：[issues/I531S7]图表联动，当子表没有数据时，会弹窗数据为空，且子表的数据也会恢复设计时候的默认数据--------------
                        xs.updateChart(linkChartId ,chartOptions);
                    }
                }
            })
...
/**
 * 获取图表钻取的数据
 * @param linkData 当前钻取图表数据
 * @param param 参数
 */
function getLinkageChartData(linkData,param){
  if(linkData.parameter){
    let arr = JSON.parse(linkData.parameter);
    if(arr.length>0){
      let params = []
...
function jimuReportHref(result,param,type) {
    ...
    if (linkType == '0') {
        ...
        if (parameter.length > 0) {
          //update-begin-author:wangshuai date:20211009 for: 钻取逻辑重构，支持报表和图表钻取 
          let linkageArgument = [];
          //type 1图表钻取 2单元格钻取
          if(type == '1'){
            linkageArgument = getLinkageChartData(resultElement, param);
          }else{
            linkageArgument = rpViewInst.getLinkageArgument(resultElement, param);
          }
          //update-end-author:wangshuai date:20211009 for:  钻取逻辑重构，支持报表和图表钻取 
        ...
        if (ejectType == '0') {
          ...
          previousPage = true
          //update-begin---author:wangshuai ---date:20220406  for：[issues/881]报表下钻时返回上一页查询参数会丢失------------
          window.localStorage.setItem("params_"+ view.excelConfigId,JSON.stringify(rpViewInst.queryInfo))
          //update-end---author:wangshuai ---date:20220406  for：[issues/881]报表下钻时返回上一页查询参数会丢失------------
        }
        //拼接是否显示上一页
        if (url.indexOf("?") != -1) {
        ...
        // 钻取跳转携带token https://gitee.com/jeecg/JimuReport/issues/I4JM0I
        if(view.token){
            url+=`&token=${view.token}`
        }
    ...
    if (linkType == '1') {
      ...
      if (parameter.length > 0) {
        //update-begin-author:wangshuai--date:20220302--for:[issues/I4VAZR]饼状图配置网络报表，参数传递不过去
        let linkageArgument = [];
        //type 1图表钻取 2单元格钻取
        if(type == '1'){
          linkageArgument = getLinkageChartData(resultElement, param);
        ...
        //update-end-author:wangshuai--date:20220302--for:[issues/I4VAZR]饼状图配置网络报表，参数传递不过去
      ...
      if(initExpression){
        ...
        if (reg.test(text)) {
         text = Number(text)
          ...
          //update-begin---author:wangshuai ---date:20220630  for：[issues/1089]钻取联动条件框取值问题------------
          if(text != "''" && text){
            text = "'" + text + "'"
          }else{
            text = null;
          }
          //update-end---author:wangshuai ---date:20220630  for：[issues/1089]钻取联动条件框取值问题------------
        }
  ...
  if(type == '1'){
    ...
    if (linkData.parameter) {
      ...
      if (arr.length > 0 && subStr) {
        ...
        //update-begin---author:wangshuai ---date: 20220701 for：[issues/1093]钻取报表添加条件后点击进入超链接报错------------
        ...
        //update-end---author:wangshuai ---date:20220701  for：[issues/1093]钻取报表添加条件后点击进入超链接报错--------------
        }
      }
    }
    if(subStr == 'text'){
