...
public abstract class InnerMQService<T> {
	...
	public void publish(BizEvent<T> bizEvent) throws Exception {
		if (bizEvent != null) {
			innerMQ.getRingBuffer()
				.publishEvent((EventTranslatorOneArg<BizEvent<T>, T>) (event, sequence, payload) -> {
	...
	@PostConstruct
	private void init() {
		innerMQ = new Disruptor<>(BizEvent::new, RINGBUFFER_SIZE,
				new CustomizableThreadFactory("risun-inner-mq-event-"), ProducerType.MULTI, new BlockingWaitStrategy());
		// 处理业务事件消费逻辑
		bizConsume();
